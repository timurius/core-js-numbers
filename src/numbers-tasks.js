...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vector1Len = Math.sqrt(x1 ** 2 + y1 ** 2);
  const vector2Len = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos((x1 * x2 + y1 * y2) / (vector1Len * vector2Len));
}
 ...
 *   1678, 3  => 2000
 */
function roundToPowerOfTen(x, pow) {
  return Math.round(x / 10 ** pow) * 10 ** pow;
}
...
function toNumber(x, def) {
  return Number.isNaN(Number(x)) || x === null ? def : x;
}
...
function getFibonacciNumber(x) {
  const fibonacciNum = [0, 1];
  if (x < 2) {
    return x;
  }
  for (let i = 2; i <= x; i += 1) {
    fibonacciNum.push(fibonacciNum[i - 1] + fibonacciNum[i - 2]);
  }
  return fibonacciNum.at(-1);
}
 ...
 *   1  => 1
 */
function getSumToN(x) {
  if (x === 1) {
    return 1;
  }
  return getSumToN(x - 1) + x;
}
...
function isPowerOfTwo(x) {
  ...
  return x === 2 ? true : isPowerOfTwo(x / 2);
}
 ...
 * 2, 2    => '10'
 */
function numberToStringInBase(x, base) {
